...
signed main(){
    ...
    vector dp(LOG+1, vector(2, vector(2, vector(2, mint(0)))));
    dp[LOG][0][0][0] = 1;
    rrep(i,LOG,0){
        ll Li = (l >> i) & 1;
        ll Ri = (r >> i) & 1;

        rep(j,0,2) rep(k,0,2) rep(m,0,2) rep(x,0,2) rep(y,0,2){
            ll _i = i;
            ll _j = j;
            ll _k = k;
            ll _m = m;
    ...
    // debug(dp);
    mint ans = 0;
    rep(k,0,2)rep(m,0,2)ans += dp[0][1][k][m];
