...
signed main() {
    ...
    sort(all(v));
    vector<ll> xused(h), yused(w);
    using S = tuple<ll, ll, ll>;
    vector<S> a;
    for(auto [x, y] : v){
        if (xused[x] && yused[y]) continue;
        if(x == 0){
            rep(j, y + 1, w) {
                a.emplace_back(make_tuple(0, j, 2));
                yused[j] = 1;
            }
        }
        ...
        a.emplace_back(make_tuple(x, y, state));
        xused[x] = 1;
        yused[y] = 1;
        if (y == 0) {
            rep(j, x + 1, h){
                a.emplace_back(make_tuple(j, 0, 1));
                xused[j] = 1;
            }
        }
        }
...
#ifdef LOCAL
        for (auto [x, y, state] : a) debug(x, y, state);
#endif

        auto f = [](ll a, ll b) { return a + b; };
        SegmentTree<ll> seg(w, f, 0);

        ll ans = h * w;
        for (auto &[x, y, state] : a) {
            if (state & 1) ans -= seg.query(y + 1, w) + 1;
            if ((state >> 1) & 1) seg.update(y, 1);
    }
