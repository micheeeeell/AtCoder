...
signed main(){
    ...
    ll n,m,l; cin >> n >> m >> l;
    ll nd = n - l;
    vector dp(nd + 1, vector<mint>(m + 1));

    dp[0][0] = 1;
    rep(i, 0, nd + 1) dp[i][0] = bc.com(nd, i);

    rep(i, 0, nd + 1) {
        rep(k, max(0LL, i - l + 1), i-1) {
            rep(j,0,m){
                ll num = i - k;
                ll j_ = j + num - 1;
                if (j_ > m) continue;
                mint tmp = dp[k][j] * bc.com(nd - k, num) * bc.fact(num);
                if(num == 1){
                    dp[i][j_] += tmp;
                    continue;
                }
    ...
    while (n >= 0) {
        ...
        cnt += l;
        num++;
        tmp *= bc.com(cnt, cnt - l);
        mint tr = bc.fact(l) / 2;
        mint tc = bc.fact(l) / (l * 2);
        rep(i, 0, num + 1) {
            ll r = i * l + (num - i) * (l - 1);
            if (r > m) break;
            mint ta = modpow(tr, num - i) * modpow(tc, i);
            ans += ta * dp[n][m - r] * tmp;
        }
