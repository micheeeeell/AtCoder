...
signed main(){
    ...
    vector ue(h + 1, vector<mint>(w + 1));
    vector<vector<mint>> yoko = ue, naname = ue;
    ...
    rep(i,0,h)rep(j,0,w){
        if (s[i][j] == '#') continue;
        if (i) dp[i][j] += ue[i][j];
        if (j) dp[i][j] += yoko[i][j];
        if (i && j) dp[i][j] += naname[i][j];
        ue[i + 1][j] = ue[i][j] + dp[i][j];
        yoko[i][j + 1] = yoko[i][j] + dp[i][j];
        naname[i + 1][j + 1] = naname[i][j] + dp[i][j];
    }
