...
signed main(){
    ...
    bool is_plus = true, is_zero = false;
    ll pl = 0, mi = 0;
    rep(i,0,n){
        if(a[i] > 0)pl++;
        if(a[i] < 0)mi++;
    ...
    if(k == mi + pl && (mi & 1))is_plus = false;
    if((k & 1) && pl == 0)is_plus = 0;
    ...
    if(k % 2 == 0){
        ...
        while(k){
            ll tm = 0, tp = 0;
            if(mid + 1 < m.size())tm = m[mid] * m[mid+1];
            if(pid + 1 < p.size())tp = p[pid] * p[pid+1];
            if(tm == max(tm, tp))mid += 2;
            else if(tp == max(tm, tp))pid += 2;
            ans *= mint(max(tm, tp));
            k -= 2;
        }
