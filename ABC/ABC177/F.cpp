...
signed main(){
    ...
    auto g = [](ll a, ll x, int num) { return (x == -1 ? a : x); };
    auto h = [](ll a, ll b) { return b; };
    auto c = [](ll x){return x < INF;};
    ...
    rep(i,0,H){
        ...
        if(b == W){
            ...
            ll id = seg.find_last(b, c);
            debug(id);
            if(id == -1 || id < a){
                seg.update(a, b, INF);
            }
            else{
                ll num = seg.query(id, id+1);
                num += b - id;
                chmin(num, seg.query(b, b + 1));
                seg.update(b, b+1, num);
                seg.update(a, b, INF);
            }
        }

        debug(seg);
        ll ans = seg.query(0, W);
        cout << (ans != INF ? ans + base : -1) << "\n";
        base++;
    }
}