...
signed main(){
    ...
    auto f = [](Pld a, Pld b) {
        return Pld{a.first + b.first, a.second+ b.second};
    };
    SegmentTree<Pld> seg(n + m, f, {0, 0});
    rrep(i,n,0){
        if(st.count(i)){
            seg.update(i, {1, 0});
            continue;
        ...
        auto p = seg.query(i + 1, i + m + 1);
        seg.update(i, {p.first / ld(m), p.second / ld(m) + 1});
    }

    debug(seg);
    auto p = seg.query(0, 1);
    cout << fixed << setprecision(10);
    cout << p.second / (ld(1) - p.first) << endl;
}