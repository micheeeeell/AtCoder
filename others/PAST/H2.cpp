...
int main(){
    ...
    auto g = [](ll a, ll x){ return a - x; };
    auto h = [](ll a, ll b){ return a + b; };
    LazySegmentTree< ll > seg(a, f, g, h, INF, 0);
    seg.build(a);
    // debug(seg.query(0,n));
    rep(i,q){
        ll t; cin >> t;
        if(t == 1){
            ...
            if(seg.query(x-1, x) >= num){
                // debug(seg.query(x-1, x));
                seg.update(x-1, x, num);
                ans += num;
            }
        }
        else if(t == 2){
            ll num; cin >> num;
            if(seg.query(0,ev) >= num){
                // debug(seg.query(0,ev));
                seg.update(0,ev,num);
                ans += num * ev;
            }
        }
