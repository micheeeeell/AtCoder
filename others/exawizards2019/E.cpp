...
signed main(){
    ...
    ll b,w; cin >> b >> w;
    vector<mint> pw(b+1), pb(w+1);
    pw[0] = modpow(mint(2), -w);
    pb[0] = modpow(mint(2), -b);
    mint t = modpow(mint(2), -1);
    rep(i,0,b){
        pw[i+1] = pw[i] + bc.com(w+i, i+1) * modpow(mint(2), -(w+i+1));
    }
    rep(i,0,w){
        pb[i+1] = pb[i] + bc.com(b+i, i+1) * modpow(mint(2), -(b+i+1));
    }
    debug(pw, pb);

    rep(i,1,b+w+1){
        mint ans = t;
        if(i-1 >= w) ans += pw[i-1-w] * t;
        if(i-1 >= b) ans -= pb[i-1-b] * t;
        cout << ans << "\n";
    }
}