...
SN *val_insert(ll x, SN *root) {
    ...
    if(cur->value > x)
        cur->left = ins, ins->parent = cur;
    else
        cur->right = ins, ins->parent = cur;
    cur->update();
    ins->splay();
    return ins;
}
...
int max_depth(SN* root){
    using p = tuple<SN*, int, string>;
    queue<p> que;
    que.emplace(root, 1, "r");
    ll res = 1;
    while(!que.empty()){
        auto [r, d, c] = que.front();que.pop();
        debug(r->value, d, c);
        if(r->left){
            que.emplace(r->left, d+1, "l");
        }
        if(r->right){
            que.emplace(r->right, d+1, "r");
        }
        res = d;
    }
...
signed main(){
    ...
    iota(all(v), 0);
    reverse(all(v));
    SplayNode *root = new SN;
    root->value = INF;
    ll num = 0;
    random_device rnd;
    mt19937 mt(rnd());
    for(auto &i : v) {
        auto [r, ok] = upper_bound(10000, root);
        root = r;
        debug(root->value, ok);

        root = val_insert(i, root);
        ll t = mt() % ++num;
        root = get(t, root);
        debug(t, max_depth(root));
    }
}