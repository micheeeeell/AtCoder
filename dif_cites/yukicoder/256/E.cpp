...
signed main(){
    ...
    vector dp(n+1, vector(n_max, vector<mint>(k+1)));
    rep(i,0,n_max)dp[1][i][0] = 1;
    rep(i,1,n){
        rep(j,0,m){
            auto [f,t,c] = v[j];
            rep(l,0,k+1){
                if(l + c <= k)dp[i+1][t][l + c] += dp[i][f][l];
            }
        }
    }
    debug(dp);
    mint ans = 0;
    rep(i,0,n_max)ans += dp[n][i][k];
    cout << ans << "\n";
}