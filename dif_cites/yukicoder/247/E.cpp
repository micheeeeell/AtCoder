...
signed main(){
    ...
    // print(table[0]);
    rep(l,31){
        rep(i,m)rep(j,m)rep(a,m){
            table[l+1][i][j] += table[l][i][a] * table[l][a][j];
        }
    }
    // print(table[0]);
    vector<mint> ans(m), nxt(m), clear(m);
    ans[0] = 1;
    rrep(l,32){
        if((k >> l) & 1){
            // debug(l);
            rep(i,m)rep(j,m)nxt[j] += ans[i] * table[l][i][j];
            ans = nxt;
            nxt = clear;
        }
        // print(nxt);
