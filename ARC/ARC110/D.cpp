...
mint solve(ll n, ll M, vl &a) {
    vl sum(n + 1);
    rep(i, 0, n) sum[i + 1] = sum[i] + a[i];
    vector dp(n, vector<mint>(M + 1));

    auto dfs = [&](auto self, ll i, ll m) -> mint {
        if (sum[n] - sum[i] > m) return 0;
        if (i == n) {
            return 1;
        }
        if (dp[i][m].a) return dp[i][m];
        
        mint res = 0;
        rep(j, a[i], m + 1) {
            mint tmp = g_com(j, a[i]) * self(self, i + 1, m - j);
            if (tmp.a == 0) break;
            res += tmp;
        }
        return dp[i][m] = res;
