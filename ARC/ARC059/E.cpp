...
signed main(){
    ...
    rep(i,0,n)cin >> a[i];
    rep(i,0,n)cin >> b[i];
    vector table(c+1, vector<mint>(n_max, 0));
    rep(i,1,n_max){
        mint tmp = 1;
        rep(j,0,c+1){
            table[j][i] = tmp + table[j][i-1];
            tmp *= i;
        }
    }
    vector dp(n+1, vector<mint>(c+1));
    dp[0][0] = 1;
    rep(i,0,n){
        rep(j,0,c+1){
            rep(k,0,j+1){
                dp[i+1][j] += dp[i][k] * (table[j-k][b[i]] - table[j-k][a[i]-1]);
            }
        }
    }
    debug(dp);
