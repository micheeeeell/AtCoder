...
signed main(){
    ...
    rep(i,0,n){
        ll t = s[i] - 'a';
        cnt[t].emplace_back(i);
    }
    ll num = 0;
    rep(i,0,26)if(cnt[i].size() & 1)num++;
    if(num != (n & 1)){
        cout << -1 << endl;
        return 0;
    }
    ...
    auto calc = [&](string &s){
        ...
        vector<ll> id(n, -1), top(26, 0);
        rep(i,0,26){
            ll sz = cnt[i].size();
            rep(j,0,sz){
                if(j < sz / 2)id[cnt[i][j]] = 1;
                else id[cnt[i][j]] = -1;
            }
        }
        debug(id);
        ...
        rep(i,0,26){
            ll sz = cnt[i].size();
            rep(j,(sz+1)/2,sz){
                id[cnt[i][j]] = n - id[cnt[i][sz-j-1]] - 1;
            }
        }
        debug(id);
        if(n & 1){
            rep(i,0,n)if(id[i] == -1){id[i] = n / 2; break;}
        }
        vector<Pll> v(n);
        rep(i,0,n)v[i] = {id[i], i};
        sort(all(v));
        debug(v);
        SegmentTree<ll> seg(n, [&](ll a, ll b){return a + b;}, 0);
        ll ans = 0;
        rep(i,0,n){
            ans += seg.query(v[i].second, n);
