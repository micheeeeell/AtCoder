...
struct SuffixArray {
    ...
    SuffixArray(const string &str_)
        : str(str_),
          n(str_.size()),
          seg(
              n + 1, [](int a, int b) { return min(a, b); },
    ...
    // sparce table を組み合わせることによって任意のi,jについて、lcp(i,j) :=
    // i,jの共通接頭辞長が計算できる セグメント木でも可能（RMQ） calc lcp int
...
signed main() {
    ...
    auto comp = [&](const auto a, const auto b) {
        ll l = sa.getLCP(a, b);
        if (l < n - max(a, b)) {
            return sa.rsa[a] < sa.rsa[b];
        }
        ll x = n - max(a, b) + min(a, b);
        if (a < b) {
            return sa.rsa[x] < sa.rsa[a];
        } else {
            return sa.rsa[b] < sa.rsa[x];
        }
    };
