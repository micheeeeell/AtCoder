...
signed main(){
    ...
    vector<ll> sub(n - 1);
    rep(i, 0, n - 1) sub[i] = a[i + 1] - a[i];
    SegmentTree<ll> seg(n - 1, f, 0);
    seg.build(sub);

    auto f2 = [](ll a, ll b) { return max(a, b); };
    auto g2 = [](ll a, ll b) { return a + b; };
    auto h2 = [](ll a, ll b) { return a + b; };
    LazySegmentTree<ll, ll> ma(n, f2, g2, h2, 0, 0);
    ma.build(a);
    ll m;
    cin >> m;
    rep(_,0,m){
        ...
        l--;
        if(t == 0){
            ll num = seg.query(l, r-1);
            ll max_ = ma.query(l, r);
            ...
            ma.update(l, r, t);
            if(l != 0) seg.update(l - 1, t);
            r--;
            if(r != n - 1) seg.update(r, -t);
        }
    }
}